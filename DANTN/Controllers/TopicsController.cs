...
public class TopicsController : ControllerBase
{
  ...
  {
    var topicByLevel = await _context.Topics.Where(x => x.LevelId == levelId).ToListAsync();

    // var query = from j in _context.Topics
    //             where j.TopicId == topicByLevel.
    //             select new { j };

    var getLevelName = await _context.Levels.FindAsync(levelId);

    var topic = topicByLevel
