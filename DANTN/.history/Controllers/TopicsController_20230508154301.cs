...
public class TopicsController : ControllerBase
{
  ...
  public async Task<ApiResult<IEnumerable<TopicViewModel>>> GetByLevel(int levelId)
  {
    var topicByLevel = await _context.Topics.FirstOrDefaultAsync(x => x.Level.LevelId == levelId);

    var topic = new TopicViewModelLevel()
    {
      TopicId = topicByLevel.TopicId,
      NameTopic = topicByLevel.NameTopic,
      TopicDesc = topicByLevel.TopicDesc,
      TopicImage = topicByLevel.TopicImage,
      LevelName = topicByLevel.Level.LevelName
    }
