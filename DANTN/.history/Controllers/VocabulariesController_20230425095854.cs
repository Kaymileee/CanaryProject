...
namespace DANTN.Controllers
{
  ...
  public class VocabulariesController : ControllerBase
  {
    ...
    public async Task<IActionResult> AddVoc([FromBody] VocabularyRequest request)
    {
    ...
    public async Task<ApiResult<VocabularyViewModel>> GetVocabulariesByTopicId(string topicId)
    {
      var vocabularies = await _context.Vocabularies.Where(v => v.TopicId == topicId).Select(x => new VocabularyViewModel()).ToListAsync();
      return new ApiSuccessResult<IEnumerable<VocabularyViewModel>>(vocabularies ||[]).ToListAsync();
